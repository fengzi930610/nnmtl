...
class Index extends kernel
{
	...
	protected function _init()
	{
		...
		$this->auth->token();
		$this->uid = $this->auth->staff_id;
		$this->uname = $this->auth->name;
		$this->MEMBER = $this->auth->staff;
	}
	...
	protected function _route($uri = NULL)
	{
		...
		$request["host"] = $host = $_SERVER["HTTP_HOST"] . "/staff";
		$request_url = explode("/", $request["ctl"]);
		unset($request_url[0]);
		$request["ctl"] = trim(implode("/", $request_url), "/");

		if (empty($request["ctl"])) {
			$request["uri"] = "";
		}
	...
	{
		if (substr($ctl, 0, 12) == "ucenter/shop") {
			Import::C("ucenter/shop");
		}
		else if (substr($ctl, 0, 8) == "ucenter/") {
	...
	{
		return K::M("helper/link")->mklink("$ctl", $args, $params, "staff", true, $extname);
	}
}
