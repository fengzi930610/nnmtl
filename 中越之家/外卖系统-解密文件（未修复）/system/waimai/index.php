...
class Index extends kernel
{
	...
	protected function _route($uri = NULL)
	{
		...
		$request["host"] = $host = $_SERVER["HTTP_HOST"] . "/" . __APP__;
		...
		if ($UxLocation = $this->cookie->get("UxLocation")) {
			list($lat, $lng) = explode(",", $UxLocation);
			$request["UxLocation"] = array("lng" => $lng, "lat" => $lat);
		}
