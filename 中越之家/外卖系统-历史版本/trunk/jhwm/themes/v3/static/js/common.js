...
Cookie.set = function(key, value, expire, path){
    path = path || '/';
    var C_DOMAIN = window.CFG['C_DOMAIN'];
...
function setUxLocation(uxl){
    ...
    Cookie.set('UxLocation','{"lat":"'+UxLocation['lat']+'","lng":"'+UxLocation['lng']+'"}', 86400*30,"/");
}
...
function getUxLocation(callback){
    ...
    if(UxLocation.lat && UxLocation.lng && UxLocation.addr){
        ...
        UxLocation = Cookie.get("UxLocation");
        Widget.MsgBox.hide();
        UxLocation["error"] = 0;
        callback(UxLocation);
        return true;
...
function geocoder(lng, lat, callback){
    ...
    $.getScript("https://api.map.baidu.com/geocoder/v2/?ak=824a595f958e444b737a5bc6325ad44f&callback="+callfun+"&location="+lat+","+lng+"&output=json&pois=1");
}
...
function placeapi(key, city, callback){
    ...
    var callfun = GGUID();
    window[callfun] = function(ret){callback(ret);}
    $.getScript("https://api.map.baidu.com/place/v2/search?ak=824a595f958e444b737a5bc6325ad44f&output=json&callback="+callfun+"&query="+key+"&page_size=10&page_num=0&scope=1&region="+city);
}
