...
//保存购物车信息
ECart.prototype.save = function(uptTime){

    this.construct();

    this.cart_list[this.shop_id] = this.shop_cart;
    if(uptTime)
        this.cart_list[this.shop_id].upt_time = (new Date()).getTime();
    ...
    for(var sid in this.cart_list){
        ...
        if(typeof(this.cart_list[sid]) == 'object'){            
            for(var pid in this.cart_list[sid]){
                ...
                if(typeof this.cart_list[sid][pid]['sku_id'] !== "undefined")
...
ECart.prototype.add = function(sku_id, num, info,type,attrs){
    ...
    this.shop_cart[sku_id].upt_time = (new Date()).getTime();
