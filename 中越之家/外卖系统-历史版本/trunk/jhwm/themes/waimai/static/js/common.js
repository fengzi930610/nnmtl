...
(function(){
    ...
    if(___orgPos)
    {
        ...
        if(___orgPos)
        {
            UxLocation = ___orgPos;
            if(typeof UxLocation.addr === "string")
                UxLocation.addr = decodeURI(UxLocation.addr);
        }
    }
})();
...
function setUxLocation(uxl){
   ...
   // localStorage['UxLocation'] = JSON.stringify(UxLocation); --原来的
    ...
    var cookieStr = "";
    if(typeof UxLocation.lat !== "undefined" && typeof UxLocation.lng !== "undefined")
        cookieStr = UxLocation.lat+","+UxLocation.lng;
    Cookie.set('UxLocation',cookieStr, 86400,"/");
}

//获取当前坐标（新函数）
function getUxLocation(cbk){
    if(typeof cbk !== "function")
        cbk = function(){};
    getUxLocationPoint(function(res){
        if(res.error === 0 && (typeof res.addr!=="string" || res.addr === ""))
        {
            UxLocation.error = 1;
            geocoder(UxLocation.lng, UxLocation.lat, function(ret){
                if(ret.error === 0)
                {
                    if(typeof ret.address === "string")
                        UxLocation.addr = $.trim(ret.address);
                }
                if(UxLocation.addr === "")
                    UxLocation.addr = "解释地址失败" + ret.address;
                UxLocation.error = 0;
    ...
    //     if(typeof UxLocation !== "undefined" && typeof UxLocation.lng !== "undefined" && typeof UxLocation.lat !== "undefined" && typeof UxLocation.addr !== "undefined")
    //     {
    //         if(typeof UxLocation.lng !== "number")
    //             UxLocation.lng = Number(UxLocation.lng);
    //         if(isNaN(UxLocation.lng) || UxLocation.lng<-180 || UxLocation.lng>180)
    //             UxLocation.lng = undefined;
    //         if(typeof UxLocation.lat !== "number")
    //             UxLocation.lat = Number(UxLocation.lat);
    //         if(isNaN(UxLocation.lat) || UxLocation.lat<-90 || UxLocation.lat>90)
    //             UxLocation.lat = undefined;
    //         if(typeof UxLocation.addr !== "string" || $.trim(UxLocation.addr) === "")
    //             UxLocation.addr = undefined;
    //         else
    //             UxLocation.addr = $.trim(UxLocation.addr);
    //         if(UxLocation.lng !== undefined && UxLocation.lat !== undefined && UxLocation.addr !== undefined)
    //         {
    //             UxLocation.error = 0;
    //             UxLocation.message = "";
    //             return true;
    ...
    // if(checkLocation())
    // {
    //     window.setTimeout(function(){ cbk(UxLocation); },1);
    //     return;
    // }
    // var retryCount = 0;
    // var retryMax = 5;
    ...
    //     message: ""
    // };
    // var posByWx = function(){debug.addInfo("微信定位");
    //     wx.getLocation({
    //         type: 'wgs84', //坐标系：gcj02为国内使用的坐标系(如腾讯、高德），wgs84为国际标准使用的坐标系（如谷歌、GPS芯片返回的数据）
    //         success: function (res) {
    //             var pointInfo = GpsToBaidu(res.longitude, res.latitude);debug.addInfo("微信定位成功");debug.addInfo(pointInfo);
    //             UxLocation.lat = Number(pointInfo.lat);
    //             UxLocation.lng = Number(pointInfo.lng);
    //             if(isNaN(UxLocation.lat) || UxLocation.lat<-90 || UxLocation.lat>90)
    //                 UxLocation.lat = undefined;
    //             if(isNaN(UxLocation.lng) || UxLocation.lng<-180 || UxLocation.lng>180)
    //                 UxLocation.lng = undefined;
    ...
    //             // UxLocation["lat"] = poi.lat.toFixed(6);
    //             // UxLocation["lng"] = poi.lng.toFixed(6);
    //             // setUxLocation(UxLocation);
    //             // geocoder(poi.lng, poi.lat, function(ret){
    //             //     Widget.MsgBox.hide();
    //             //     if(ret.error === 0)
    //             //     {
    ...
    //             //     else
    //             //     {
    //             //         Widget.MsgBox.alert("获取地理位置失败"+ret.address);
    ...
    //             {
    //                 UxLocation.message = "定位失败";
    //                 cbk(UxLocation);
    //             }
    //             else
    //             {
    //                 ++retryCount;
    //                 //原代码会尝试使用IP进行定位，但目前没有，所以只是进行简单的重试
    //                 window.setTimeout(posByWx,1);
    //             }
    //         },
    //         cancel: function (res) { 
    //             UxLocation.message = "拒绝授权，获取位置失败";
    //             cbk(UxLocation);
    //         }
    //     });
    // };
    // var posByBrower = function(){debug.addInfo("浏览器定位-"+retryCount);
    ...
    //             var pointInfo = GpsToBaidu(position.coords.longitude, position.coords.latitude);debug.addInfo("浏览器定位成功");debug.addInfo(pointInfo);
    //             UxLocation.lat = Number(pointInfo.lat);
    //             UxLocation.lng = Number(pointInfo.lng);
    //             if(isNaN(UxLocation.lat) || UxLocation.lat<-90 || UxLocation.lat>90)
    //                 UxLocation.lat = undefined;
    ...
    //                 }
    //             });
    //         }, function(e) {debug.addInfo("浏览器定位 error - "+e.toString());
    //             if(retryCount >= retryMax)
    //             {
    //                 UxLocation.message = "定位失败";
    //                 cbk(UxLocation);
    ...
    //             {
    //                 ++retryCount;
    //                 window.setTimeout(posByBrower,1);
    //             }
    //         },{
    ...
    //     {
    //         //这里可以尝试使用IP定位，但目前没有
    //         UxLocation.message = "您的浏览器不支持定位功能";
    //         cbk(UxLocation);
    //     }
    // };
    // if(checkIsWeixin())
...
//获取坐标点，但不进行地理位置解码
function getUxLocationPoint(cbk)
{
    if(typeof cbk !== "function")
        cbk = function(){};
    var checkLocation = function(){
    ...
    var posByWx = function(){if(typeof debug !== "undefined") debug.addInfo("微信定位");
        wx.getLocation({
            type: 'wgs84', //坐标系：gcj02为国内使用的坐标系(如腾讯、高德），wgs84为国际标准使用的坐标系（如谷歌、GPS芯片返回的数据）
            success: function (res) {
                var pointInfo = GpsToBaidu(res.longitude, res.latitude);if(typeof debug !== "undefined") debug.addInfo("微信定位成功");if(typeof debug !== "undefined") debug.addInfo(pointInfo);
                UxLocation.lat = Number(pointInfo.lat);
                UxLocation.lng = Number(pointInfo.lng);
                if(isNaN(UxLocation.lat) || UxLocation.lat<-90 || UxLocation.lat>90)
                    UxLocation.lat = undefined;
                if(isNaN(UxLocation.lng) || UxLocation.lng<-180 || UxLocation.lng>180)
                    UxLocation.lng = undefined;
                if(typeof UxLocation.addr === "undefined")
                    UxLocation.addr = "";
                UxLocation.error = 0;
                setUxLocation(UxLocation);
                cbk(UxLocation);
    ...
    var posByBrower = function(){if(typeof debug !== "undefined") debug.addInfo("浏览器定位-"+retryCount);
        if(typeof navigator!=="undefined" && typeof navigator.geolocation !=="undefined" && typeof navigator.geolocation.getCurrentPosition === "function")
        {
            navigator.geolocation.getCurrentPosition(function(position) {
                var pointInfo = GpsToBaidu(position.coords.longitude, position.coords.latitude);if(typeof debug !== "undefined") debug.addInfo("浏览器定位成功");if(typeof debug !== "undefined") debug.addInfo(pointInfo);
                UxLocation.lat = Number(pointInfo.lat);
                UxLocation.lng = Number(pointInfo.lng);
                if(isNaN(UxLocation.lat) || UxLocation.lat<-90 || UxLocation.lat>90)
                    UxLocation.lat = undefined;
                ...
                setUxLocation(UxLocation);
                cbk(UxLocation);
            }, function(e) {if(typeof debug !== "undefined") debug.addInfo("浏览器定位 error - "+e.toString());
                if(retryCount >= retryMax)
                {
                    UxLocation.message = "定位失败";
                    cbk(UxLocation);
...
//取到当前的坐标(Biadu系坐标)(旧函数)
function getUxLocation_Old(callback){
    ...
    if(checkIsWeixin()){ //微信获位置坐标
        var it_wxloction = setInterval(function () {
            wx.ready(function(){
                wx.getLocation({
                    type: 'gcj02', //将地图坐标系由国标gcj02改为国际标准wgs84
                    success: function (res) {
...
var googleJsLoading = false;
var googleLoaded = false;
var googleLoadingFns = [];
var googleLoadInfo = null;
var googleLoadRetryCount = 0;
function loadGoogleJs(cbk)
{
    if(googleLoaded)
    {
        if(typeof cbk === "function")
            cbk(googleLoadInfo.res,googleLoadInfo.status);
        return;
    }
    if(typeof cbk === "function")
        googleLoadingFns.push(cbk);
    if(googleJsLoading)
        return;
    googleJsLoading = true;
    var loadDone = function(){
        googleLoaded = true;
        for(var k in googleLoadingFns)
            googleLoadingFns[k](googleLoadInfo.res,googleLoadInfo.status);
        googleLoadingFns = [];
    };
    if(CFG && CFG.JH_MAP_KEY)
    {
        var loadFn = function(){
            $.getScript("https://maps.googleapis.com/maps/api/js?key="+CFG.JH_MAP_KEY+"&language=zh-CN&region=VN&libraries=places",function(res,status){
                if(status === "success" || googleLoadRetryCount >= 10)
                {
                    googleLoadInfo = {
                        res:res,
                        status:status
                ...
                {
                    ++googleLoadRetryCount;if(typeof debug !== "undefined") debug.addInfo("载入地图API失败，重试 - " + googleLoadRetryCount);
                    window.setTimeout(loadFn,100);
                }
            });
        };
        loadFn();
            ...
            srv.geocode({location: {lng:this.lng,lat:this.lat}}, (function(results, status) {
                ...
                if(status === 'OK')
                {
                    if(typeof results[0] !== "undefined")
                    {
                        ...
                        var reAddrArr = addrComponent.arrAddr.reverse();
                        cbkData.address = reAddrArr.join(" ") + "," + addrComponent.city;
                        cbkData.address = cbkData.address.replace(/\s+/ig," ");
                        cbkData.city = addrComponent.province;//越南城市是一级行政区
                    }
                    else
                        cbkData.address = "Empty result";
                }
                else
                    cbkData.address = status;
                this.cbk(cbkData);
            }).bind(this));
        }
        else
            alert("地图API失败"+status);
