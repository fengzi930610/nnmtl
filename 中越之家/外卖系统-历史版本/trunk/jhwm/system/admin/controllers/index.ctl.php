...
class Ctl_Index extends Ctl
{
    ...
    public function index()
    {
        ...
        //2019-03-04 添加 把所有菜单的参数记录读取并放置到页面中
        $ids = K::M('module/params')->get_menu_tree_ids($top_menu);
        $modParams = [];
        if($ids)
        {
            $modParams = K::M('module/params')->items_group_modid(['mod_id'=>"IN:".implode(",", $ids),NULL,1,999999]);
            if(!$modParams)
                $modParams = [];
        }
        $ids = K::M('module/params')->get_menu_tree_ids($tree['menu']);
        if($ids)
        {
            $tArr = K::M('module/params')->items_group_modid(['mod_id'=>"IN:".implode(",", $ids),NULL,1,999999]);
            if($tArr)
            {
                foreach($tArr as $modId => &$item)
                {
                    if(!isset($modParams[$modId]))
                        $modParams[$modId] = $item;
                    else
                    {
                        $modParams[$modId] = [];
                        foreach($item as $kk => $vv)
                            $modParams[$modId][$kk] = $vv;
                    }
                }
            }
            unset($tArr);
        }
        $this->pagedata['param_menu'] = $modParams;
        //==============================================
    ...
    public function srvapi()
    {
        $api = $this->GP("api");
        if($api === "geocode")
        {
            $lng = (float)$this->GP("lng");
            $lat = (float)$this->GP("lat");
            if(!$lng || !$lat)
                $this->msgbox->add("未设置坐标",1);
            else
            {
                $info = K::M("magic/baidu")->decode_by_location($lng,$lat);
                if(!$info)
                    $this->msgbox->add("解释失败",1);
                else
                    $this->msgbox->add($info['province'].$info['city'].$info['addr']."|".$info['lng'].",".$info['lat']);
            }
        }
        else if($api === "regeocode")
        {
            $key = trim($this->GP("key"));
            $city = trim($this->GP("city"));
            if($key ==="")
                $this->msgbox->add("未设置查询关键字",1);
            else
            {
                if($city === "")
                    $city = NULL;
                $info = K::M("magic/baidu")->geocode_by_addr($key,$city);
                if(!$info)
                    $this->msgbox->add("解释失败",1);
                else
                    $this->msgbox->add($info[0]['lng'].",".$info[0]['lat']);
            }
        }
        else
            $this->msgbox->add("未知接口",1);
    }
}