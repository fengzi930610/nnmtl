...
class Ctl_Group_Group extends Ctl {
     ...
     public function setarea($city_id = null)
    {
        ...
        //20190113 从缓存文件中查找半径设置数值
        $cache = $this->system->cache->get("pei_group_radius");
        if($cache && is_array($cache = json_decode($cache,true)))
        {
            foreach($items as $kk => &$item)
            {
                $id = $item['group_id'];
                if(isset($cache[$id]) && is_numeric($cache[$id]) && (float)$cache[$id]>0)
                    $items[$kk]['polygon_radius'] = (float)$cache[$id];
                else
                    $items[$kk]['polygon_radius'] = "";
                unset($item);
            }
        }
        //==========================================
    ...
    public function get_groups($city_id = null)
    {
        if(!$city_id = (int)$city_id){
            ...
            foreach($items as $k=>$v){
                ...
                $id = $v['group_id'];
                if(isset($cache[$id]) && is_numeric($cache[$id]) && (float)$cache[$id]>0)
                    $items[$k]['polygon_radius'] = (float)$cache[$id];
                else
                    $items[$k]['polygon_radius'] = "";
                //=====================================
            }
        ...
        if (!$group_id = (int)$group_id) {
            ...
            if (K::M('pei/group')->update($group_id, array('polygon_point'=>$polygon_point))) {
                ...
                if($this->GP("radius"))
                {
                    ...
                    if(!$cache || !is_array($cache = json_decode($cache,true)))
                        $cache = [];
                    if((float)$this->GP("radius") > 0)
                        $cache[(int)$group_id] = (float)$this->GP("radius");
                    else
                        $cache[(int)$group_id] = 0;
                    $this->system->cache->set("pei_group_radius",json_encode($cache));
                }
                //=====================================
    ...
    public function set_map($group_id){
        if(!$group_id){
            ...
            if(!$cache || !is_array($cache = json_decode($cache,true)))
                $cache = [];
            if(isset($cache[(int)$group_id]) && (float)$cache[(int)$group_id] > 0)
                $group['polygon_radius'] = (float)$cache[(int)$group_id];
            else
                $group['polygon_radius'] = "";
            //==========================================
            $this->pagedata['detail'] = $group;
