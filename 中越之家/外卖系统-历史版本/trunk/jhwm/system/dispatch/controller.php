...
class Ctl extends Factory
{
	...
	public function __construct(&$system)
	{
		...
		if ($this->auth->token()) {
			...
			$bJump = true;
			 if(in_array($this->request["ctl"], $noCheckCtl) || (isset($noCheckAct[$this->request["ctl"]]) && in_array($this->request["act"], $noCheckAct[$this->request["ctl"]]))) 
			 	$bJump = false;
			if($bJump)
			{
	...
	protected function _init_pagedata()
	{
		...
		$output->setCompileDir(__CFG::DIR . "data/tpldispatch");
	}
	...
	{
		$theme = $this->default_theme();
		$output->setTemplateDir(__CFG::TMPL_DIR . "dispatch");
	...
	{
		if ($type == "file") {
			$file = __CFG::TMPL_DIR . "dispatch" . DIRECTORY_SEPARATOR . $name;
			return $file;
		}
	...
	{
		return K::M("helper/link")->mklink("$ctl", $args, $params, "dispatch", true, $extname);
	}
}