...
class Ctl_Order extends Ctl {
    ...
    //2019-03-06 添加 生成自动完成的redis队列数据
    public function create_queue()
    {
        // if(0 === K::M('cache/redis')->queue_len("auto_complete_order_ids"))
        // {
        //逻辑：
        //  每执行一次，就会把10天前的订单加到队列
        //  只要当天执行过接口，则今天就不会再被执行！
        $lastTime = K::M('cache/redis')->get("auto_complete_order_last_create_time");
        if(!$lastTime || strtotime(date("Y-m-d",(int)$lastTime)) < strtotime(date("Y-m-d")))
        {
            // echo "create...<br>";
            $sTime = strtotime(date("Y-m-d"))-864000;
            $eTime = $sTime + 86400 - 1;
            $orders = K::M('order/order')->items([
                ':SQL' => "order_status=4 AND dateline>={$sTime} AND dateline<={$eTime}"
            ],"dateline ASC,order_id ASC,dateline ASC",1,99999999);
            if($orders)
            {
                foreach($orders as &$order)
                    K::M('cache/redis')->queue_push("auto_complete_order_ids",(int)$order['order_id']);
            }
             K::M('cache/redis')->set("auto_complete_order_last_create_time",time());
        }
        // }
        // K::M('cache/redis')->delete("auto_complete_order_last_create_time");
        die(date("Y-m-d H:i:s")."success\n");
    }

    //2019-03-06 添加 自动完成订单API接口
    public function queue_complete()
    {
        set_time_limit(0);
        ignore_user_abort(true);
        $order_id = K::M('cache/redis')->queue_pop("auto_complete_order_ids");
        if($order_id)
        {
            $order = K::M('order/order')->find(['order_id'=>$order_id]);
            if($order)
            {
                //确认结单
                //开启事务，防止一表操作失败而导致数据错误
                $this->system->db->begin();
                $optDone = false;
                if(K::M('waimai/order')->confirm($order_id,null,'system'))
                {
                    ...
                    $waimai_log = [
                        'order_id'=>$order_id,
                        'from'=>'system',
                        'log'=>'用户未确认收货，系统自动结算订单(ID:'.$order['order_id'].')',
                        'type'=>6
                    ];
                    if(K::M('waimai/log')->create($waimai_log))
                    {
                        ...
                        $datas = [
                            ...
                            'score_peisong' => 5,
                            'score' => 5,
                            'pei_time' => ceil(($order['lasttime'] - $order['dateline'])/60),
                            'shop_id' => $order['shop_id'],
                            'have_photo' => 0,
                            'contents' => "用户未及时评价，系统自动好评",
                            'is_anonymous' => 1,
                        ];

                        //商品评价
                        $hasErr = false;
                        $orderProducts = K::M('waimai/orderproduct')->items(['order_id' => $order_id],NULL,1,999999);
                        $extend = ['intro'=>""];
                        if($orderProducts)
                        {
                            foreach($orderProducts as $orderProduct)
                            {
                                if(!K::M('waimai/product')->update_count($orderProduct['product_id'], 'good', 1))
                                {
                                    $hasErr = true;
                                    break;
                                }
                        ...
                        if(!$hasErr)
                        {
                            ...
                            $waimai_format = K::M('waimai/waimai')->detail($order_id);
                            if($waimai_format)
                            {
                                if($order['pei_type'] == 3)
                                ...
                                {
                                    if(!K::M('order/order')->update($datas['order_id'], array('comment_status' => 1)))
                                        break;
                                    ...
                                    if($order['pei_type']==1&&$order['staff_id'])
                                    {
                                        ...
                                        if(!K::M('staff/staff')->update($order['staff_id'], $update_data_staff, true))
                                            $hasErr = true;
                                        if(!$hasErr && !K::M('staff/comment')->create($staff_data))
                                            $hasErr = true;
                                    }

                                    if(!$hasErr && K::M('waimai/waimai')->update($order['shop_id'], $update_data, true))
                                    {
                ...
                if($optDone)
                    $this->system->db->commit();
                else
                {
                    $this->system->db->rollback();
                    K::M('cache/redis')->queue_push("auto_complete_order_ids",$order_id);
                }
            }
        }
