...
class Ctl_Trade_Payment extends Ctl
{
    ...
    public function notify_verify($code, $from = null)
    {
        ...
        $success = false;
        if($obj = K::M('trade/payment')->loadPayment($code)){
            $bUseTrans = false; //2019-03-16 添加 开启事务
            if($trade = $obj->notify_verify()){
                if(!$log = K::M('payment/log')->log_by_no($trade['trade_no'])){
                    ...
                    if(K::M('payment/log')->set_payed($trade['trade_no'], $trade['pay_trade_no'])){
                        if($log['from'] == 'money'){
                            $success = (bool)K::M('trade/payment')->payed_money($log, $trade);
                        }else if($log['from']=='deliver'){
                            $success = (bool)K::M('trade/payment')->payed_deliver($log, $trade);
                        }else if($log['from']=='peicard'){
                            $success = (bool)K::M('trade/payment')->payed_peicard($log, $trade);
                        }else{
                            $success = (bool)K::M('trade/payment')->payed_order($log, $trade);
                        }
                    }
                }
            }
