...
{   
    ...
    protected $_cols = 'order_id,product_number,product_price,package_price,freight,spend_number,spend_status,shop_amount,roof_amount,first_shop,first_roof,freight_addone';
    ...
    public function confirm($order_id=null, $order=null, $from='member')
    {
        ...
        if(!$waimai_order = $this->detail($order_id)){
            ...
            if($this->db->update('order', array('order_status'=>8), "order_id='{$order_id}'", true)){
                ...
                try
                {
                    if(!K::M('order/time')->update($order_id,array('order_compltet_time'=>__TIME)))
                        throw new \Exception("Error", 1);
                        
                    // 基于 mysql_affected_rows 返回影响的记录行数防止并发  add by zhuhongwei 2017-3-9
                    if(!K::M('order/order')->update($order_id, array('lasttime'=>__TIME), true))
                        throw new \Exception("Error", 1);
                    if($from == 'member'&&$waimai_order['pei_type'] == 3){ //自提状态
                        if(!K::M('waimai/order')->update($order_id, array('spend_status'=>1), true))
                            throw new \Exception("Error", 1);
                    }

                    $tongji = array(
                        // 'amount'=>$order['amount']+$order['money'],
                        'amount'=>$order['amount']+$order['money']-K::M('order/order')->get_part_refunded_amount($order_id), //2019-03-12 修改 减去已退金额
                    ...
                    if($order['online_pay']){
                        ...
                        if($order['from'] == 'waimai'){
                            ...
                            if($order['pei_type']){
                                ...
                                //2019-01-22 新增 如果订单是使用运费按距离计算模式，则需要把运费减出来
                                if((int)$order['is_calc_freight_mod'] !== 0)
                                    $shop_amount -= (float)$waimai_order['freight'];    //2019-01-24 记，因为记录在订单的运费是包含附加费的，所以不需要再减出来
                                //==============================================================
                            }
                        }
                        if($staff_amount && $order['staff_id']){
                            ...
                            if (K::M('staff/bills')->create($bill_data)) {
                                ...
                                if(!K::M('staff/billslog')->create($bill_log))
                                    throw new \Exception("Error", 1);
                            }
                            else
                                throw new \Exception("Error", 1);
                            ...
                            if(!K::M('staff/log')->log($order['staff_id'],$true_staff_amount,$staff_log))
                                throw new \Exception("Error", 1);
                            //这里生成骑手补贴相关 外卖3.8
                            if($staff_amount<$true_staff_amount){
                                ...
                                if(!K::M('subsidy/staff')->create($subsidy_staff))
                                    throw new \Exception("Error Processing Request", 1);
                            }
                            //这里生成骑手补贴相关 外卖3.8
                            //K::M('staff/staff')->update_money($order['staff_id'], $staff_amount, $log);
                        }
                        //2019-03-12 新增 减去已经退部分的钱
                        $shop_amount -= K::M('order/order')->get_part_refunded_amount($order_id);
                        if($shop_amount < 0)
                            $shop_amount = 0;
                        //=====================
                        if($shop_amount){
                            ...
                            if($config['jiesuan_type']==1){
                                ...
                                $fee =  number_format((($order['total_price']-$waimai_order['freight']-$waimai_order['package_price']-K::M('order/order')->get_part_refunded_amount($order_id)) * $config['waimai_bl'])/100, 2, '.', ''); //2019-03-12 修改 减去已退款部分
                                if($fee<=0){
                                    $fee = 0;
                                }
                            }else if($config['jiesuan_type']==2){
                                // $fee =  number_format((($order['total_price']-$waimai_order['freight']) * $config['waimai_bl'])/100, 2, '.', '');
                                $fee =  number_format((($order['total_price']-$waimai_order['freight']-K::M('order/order')->get_part_refunded_amount($order_id)) * $config['waimai_bl'])/100, 2, '.', ''); //2019-03-12 修改 减去已退款部分
                                if($fee<=0){
                                    $fee = 0;
                                }
                            }else{
                            ...
                            $bill_data['shop_amount'] = $waimai_order['shop_amount']+$waimai_order['first_shop']+$order['coupon']-K::M('order/order')->get_part_refunded_amount($order_id); //2019-03-12 修改 减去已退款部分
                            ...
                            $bill_data['user_amount'] = $order['amount']+$order['money']-K::M('order/order')->get_part_refunded_amount($order_id); //2019-03-12 修改 减去已退款部分
                            if($order['pei_type']==1||$order['pei_type']==2){
                                $bill_data['freight'] = $order['pei_amount'];
                            }else{
                                $bill_data['freight'] = 0;
                            }
                            //2019-01-22 新增 如果订单是使用运费按距离计算模式，则需要把运费加到sys_freight中
                            if((int)$order['is_calc_freight_mod'] !== 0)
                            {
                                $bill_data['sys_freight'] = (float)$waimai_order['freight'] - (float)$waimai_order['freight_addone'];   //2019-01-24 原来的运费是包含附加费的，所以对账单中要把附加费减出来，以做区分
                                //2019-01-24 新增 把配送附加费，添加到freight_addone
                                $bill_data['freight_addone'] = (float)$waimai_order['freight_addone'];
                            }
                            //==============================================================
                            if(!K::M('waimai/bills')->create($bill_data))
                                throw new \Exception("Error", 1);
                            $filter = array(
                                'bills_sn'=>date('Ymd'),
                            ...
                            $data = array('bills_id'=>$bills['bills_id'],
                                ...
                                'shop_amount'=>$waimai_order['shop_amount']+$waimai_order['first_shop']+$order['coupon']-K::M('order/order')->get_part_refunded_amount($order_id), //2019-03-12 修改 减去已退款部分
                            ...
                            if((int)$order['is_calc_freight_mod'] !== 0)
                            {
                                $data['sys_freight'] = (float)$waimai_order['freight'] - (float)$waimai_order['freight_addone'];    //2019-01-24 原来的运费是包含附加费的，所以对账单中要把附加费减出来，以做区分
                                //2019-01-24 新增 把配送附加费加到freight_addone中
                                $data['freight_addone'] = (float)$waimai_order['freight_addone'];
                            }
                            //==============================================================
                            if(!K::M('waimai/billslog')->create($data))
                                throw new \Exception("Error", 1);
                            //修改对账单 --end
                            ...
                            if(($order['hongbao']>0)||($order['order_youhui'])>0||($order['first_youhui']>0)||($order['coupon']>0)||($order['discount_youhui']>0)||($order['huangou_youhui']>0)||($order['peicard_youhui']>0)){
                                ...
                                $subsidy_waimai['shop_mj'] = $waimai_order['shop_amount']-$order['discount_youhui']-K::M('order/order')->get_part_refunded_amount($order_id); //2019-03-12 修改 减去已退款部分
                                ...
                                if(!K::M('subsidy/waimai')->create($subsidy_waimai))
                                    throw new \Exception("Error", 1);
                        ...
                        //首单奖励发放
                        //订单满反--beging
                        if(false === K::M('waimai/huodongmf')->coupon_mf($order))
                            throw new \Exception("Error", 1);
                        //订单满反--end
                        ...
                        if($staff_amount){
                            ...
                            $tongji['amount'] -= K::M('order/order')->get_part_refunded_amount($order_id); //2019-03-12 添加 减去已退部分
                        ...
                        $shop_amount -= K::M('order/order')->get_part_refunded_amount($order_id); //2019-03-12 添加 减去已退部分
                        ...
                        if($shop_detail['jiesuan_type']==1){
                            ...
                            $fee =  number_format((($order['total_price']-$waimai_order['freight']-$waimai_order['package_price'] - K::M('order/order')->get_part_refunded_amount($order_id)) * $shop_detail['waimai_bl'])/100, 2, '.', ''); //2019-03-12 修改 减去已退部分
                            if($fee<=0){
                                $fee = 0;
                            }
                        }else if($shop_detail['jiesuan_type']==2){
                            // $fee =  number_format((($order['total_price']-$waimai_order['freight']) * $shop_detail['waimai_bl'])/100, 2, '.', '');
                            $fee =  number_format((($order['total_price']-$waimai_order['freight'] - K::M('order/order')->get_part_refunded_amount($order_id)) * $shop_detail['waimai_bl'])/100, 2, '.', ''); //2019-03-12 修改 减去已退部分
                            if($fee<=0){
                                $fee = 0;
                            }
                        }else{
                        ...
                        $bill_data['fee'] =$fee;
                        // $bill_data['shop_amount'] = $waimai_order['shop_amount']+$waimai_order['first_shop']+$order['coupon'];
                        $bill_data['shop_amount'] = $waimai_order['shop_amount']+$waimai_order['first_shop']+$order['coupon'] - K::M('order/order')->get_part_refunded_amount($order_id); //2019-03-12 修改 减去已退部分
                        $bill_data['roof_amount'] = $waimai_order['roof_amount']+$waimai_order['first_roof']+$order['hongbao']+$order['peicard_youhui'];
                        ...
                        $bill_data['user_amount'] = $order['total_price']-$order['first_youhui']-$order['coupon']-$order['order_youhui']-$order['hongbao']-$order['discount_youhui']-$order['huangou_youhui']-$order['peicard_youhui'] - K::M('order/order')->get_part_refunded_amount($order_id); //2019-03-12 修改 减去已退部分
                        ...
                        $data = array('bills_id'=>$bills['bills_id'],
                            ...
                            'fee'=>$fee,
                            // 'shop_amount'=>$waimai_order['shop_amount']+$waimai_order['first_shop']+$order['coupon'],
                            'shop_amount'=>$waimai_order['shop_amount']+$waimai_order['first_shop']+$order['coupon'] - K::M('order/order')->get_part_refunded_amount($order_id), //2019-03-12 修改 减去已退部分
                            'roof_amount'=>$waimai_order['roof_amount']+$waimai_order['first_roof']+$order['hongbao']+$order['peicard_youhui'],
                            'freight'=> $bill_data['freight'],
                        ...
                        $order['shop_amount'] = $order['total_price']-$order['first_youhui']-$order['coupon']-$order['order_youhui']-$order['hongbao'] - $order['discount_youhui'] - $order['huangou_youhui'] - $order['peicard_youhui'] - K::M('order/order')->get_part_refunded_amount($order_id); //2019-03-12 修改 减去已退部分
                        if(!K::M('cash/bills')->create_bills($order))
                            throw new \Exception("Error", 1);
                        if(false === K::M('waimai/huodongmf')->coupon_mf($order))
                            throw new \Exception("Error", 1);
                        //商家补贴统计
                        if(($order['hongbao']>0)||($order['order_youhui'])>0||($order['first_youhui']>0)||($order['coupon']>0)||($order['discount_youhui']>0)){
                            ...
                            $subsidy_waimai['shop_mj'] = $waimai_order['shop_amount']-$order['discount_youhui']-$order['huangou_youhui'] - K::M('order/order')->get_part_refunded_amount($order_id); //2019-03-12 修改 减去已退部分
                            $subsidy_waimai['shop_coupon'] = $order['coupon'];
                            $subsidy_waimai['year'] = date('Y',$order['dateline']);
                            $subsidy_waimai['mouth'] = date('Ym',$order['dateline']);
                            $subsidy_waimai['day'] = date('Ymd',$order['dateline']);
                        ...
                        $tongji['amount'] =  $tongji['shop_amount']  = $order['total_price']-$order['coupon']-$order['order_youhui']-$order['first_youhui']+$waimai_order['roof_amount']+$waimai_order['first_roof']-$order['discount_youhui'] - K::M('order/order')->get_part_refunded_amount($order_id); //2019-03-12 修改 减去已退部分
                        ...
                        $tongji['shop_mj'] =  $waimai_order['shop_amount']-$order['discount_youhui'] - K::M('order/order')->get_part_refunded_amount($order_id); //2019-03-12 修改 减去已退部分
                    ...
                    //生成统计数据
                    if(!K::M('site/tongji')->create($tongji))
                        throw new \Exception("Error", 1);
                    //生成统计数据
                    ...
                    if($order['staff_id'] && !$order_time['staff_compltet_time']&&$order['expect_time']<__TIME){
                        ...
                        if(!K::M('staff/timeoutorder')->create($data_time_out))
                            throw new \Exception("Error", 1);
                    }

                    //2019-03-16 加注 加入事务后，这里是处理首单给推荐人发奖励的，其中出现找不到用户之类的概率相当低，再者就算出现找不到，也只是不发奖励而已，所以除了更新数据库操作失败会抛错误外，其他情况都不作处理！
                    if($order['first_order']){
                        if($m = K::M('member/member')->detail($order['uid'])){
                            if(preg_match('/(B|S|D|M)(\d+)/i', $m['pmid'], $a)){
                                if($a[1] == 'M'){ //会员邀请
                                    if(!K::M('member/invite')->update($order['uid'],array('status'=>2)))//邀请结束
                                        throw new \Exception("Error", 1);
                                    ...
                                    if ($inviteCfg['is_inviter_hongbao'] == 1) { // 邀请人是否奖励红包
                                        if($pm = K::M('member/member')->detail((int)$a[2])){
                                            if($moneys = K::M('member/invite')->send_hongbao_by_cfg($inviteCfg['inviter_hongbao_cfg'], $pm['uid'], 5)){
                                                if(!K::M('member/invite')->update($order['uid'], array('money'=>$moneys)))
                                                    throw new \Exception("Error", 1);
                                            }
                                        }
                                    }
                                    ...
                                    if ($dituiCfg['order_amount']) { // 推荐用户首单，奖励推荐人
                                        if($dm = K::M('ditui/ditui')->detail((int)$a[2])){
                                            if(K::M('ditui/ditui')->update_money($dm['ditui_id'], $dituiCfg['order_amount'])){
                                                if(!K::M('ditui/ditui')->update_ordercount($dm['ditui_id']))
                                                    throw new \Exception("Error", 1);
                                                if($dtm = K::M('ditui/member')->find(array('ditui_id'=>$dm['ditui_id'],'uid'=>$order['uid']))){
                                                    ...
                                                    if(!K::M('ditui/member')->update($dtm['mid'],$dm_data))
                                                        throw new \Exception("Error", 1);
                                                }

                                                $intro = L('推荐用户首单奖励金额');
                                                if(!K::M('ditui/log')->log($dm['ditui_id'],$order['uid'],$dituiCfg['order_amount'],$intro,'invite'))
                                                    throw new \Exception("Error", 1);
                                            }
                                        }
                                    }
                    ...
                    if($order['online_pay'] == 1 && $order['jifen_cfg']['jifen_type'] == 2){
                        if(!K::M('jifen/jifen')->update_jifen($order_id,$order))
                            throw new \Exception("Error", 1);
                    }
                    if($from == 'admin'){
                        $log = '管理员确认订单完成';
                    ...
                    $waimai = K::M('waimai/waimai')->detail($order['shop_id']);
                    if(!K::M('order/log')->create(array('order_id'=>$order_id,'from'=>$from,'log'=>$log,'status'=>8)))
                        throw new \Exception("Error", 1);
                    if($order['order_from'] != 'wxapp' || $from != 'member'){
                        K::M('order/order')->send_member('订单已完成', sprintf("您在[%s]下的订单(%s)，{$log}", $waimai?$waimai['title']:"-", $order_id), $order);
                    }
                    //记录配送员距离等信息 2017-12-25 外卖3.7新增 begin
                    K::M('staff/distance')->create($order);
                    //记录配送员距离等信息 2017-12-25 外卖3.7新增 end

                    //2019-02-22 新增 删除订单预计配送时间的缓存
                    K::M('order/order')->delFreightTimeCache($order_id);
                    //========================================
                    
                    // return true; //2019-03-16 因为需要检测错误才决定返回，所以这里不直接返回
    ...
    public function cancel($order_id=null, $order=null, $from='member', $extend = '')
    {
        ...
        if(!$order && !($order = K::M('order/order')->detail($order_id))){
            ...
            try
            {
                $totalRefundedAmount = K::M('order/order')->get_part_refunded_amount($order_id);//2019-03-13 添加 获取已退款总额
                //-1:已取消，0：未处理，1：已接单，2：已配货，3：配送开始，4：配送完成，8：订单完成
                // 基于 mysql_affected_rows 返回影响的记录行数防止并发  add by zhuhongwei 2017-3-9
                if($this->db->update('order', array('order_status'=>-1), "order_id='{$order_id}'", true)){
                    if(!K::M('order/order')->update($order_id, array('lasttime'=>__TIME), true))// 更新时间
                        throw new \Exception("Error", 1);
                    if($order['online_pay']){
                        ...
                        if($order['pay_status'] && ($order['amount'] > 0)){
                            ...
                            if(in_array($order['pay_code'], array('wxpay', 'alipay'))){
                                ...
                                $order['refund_amount'] -= $totalRefundedAmount; //2019-03-13 添加 减去已退款部分
                        ...
                        if($refund_money > 0){
                            if(!K::M('member/member')->update_money($order['uid'], $refund_money, '订单(ID:'.$order['order_id'].')取消退回到余额'))
                                throw new \Exception("Error", 1);
                        }
                    ...
                    //退还红包
                    if($order['hongbao_id']){
                        if(!K::M('hongbao/hongbao')->update($order['hongbao_id'], array('order_id'=>0, 'used_time'=>0, 'used_ip'=>'')))
                            throw new \Exception("Error", 1);
                    }
                    //退回优惠券
                    if($order['from'] == 'waimai' && $order['coupon_id']){
                        K::M('waimai/coupon')->refund_coupon($order['coupon_id']); //2019-03-16 注 此函数无返回值
                    }
                    //退回商品库存
                    if($order['from'] == 'waimai'){
                        if(!$this->return_sku($order_id, $order))
                            throw new \Exception("Error", 1);
                    }
                    //更新商户订单数量
                    if($order['shop_id']){
                        if($order['from'] == 'waimai'){
                            if(!K::M('waimai/waimai')->update_count($order['shop_id'], 'orders', -1))
                                throw new \Exception("Error", 1);
                        }else {
                            if(!K::M('shop/shop')->update_count($order['shop_id'], 'orders', -1))
                                throw new \Exception("Error", 1);
                        }
                    }
                    //更新服务人员订单数量
                    if($order['staff_id']){
                        if(!K::M('staff/staff')->update_count($order['staff_id'], 'orders', -1))
                            throw new \Exception("Error", 1);
                    }
                    //更新会员订单数量
                    if($order['uid']){
                        if(!K::M('member/member')->update_count($order['uid'], 'orders', -1))
                            throw new \Exception("Error", 1);
                    }

                    //4.0删除会员卡记录
                    if($order['peicard_id']){
                        K::M('peicard/log')->delete_by_orderId($order['order_id']); //2019-03-16 注 不检测返回，理由同refund接口的相应代码
                    }
                    
                    //2019-03-16 加注 这里是向商家、骑手发送消息的，发送的时机应该是在确定操作是成功的之后，但因为目前并没有调度增值模块，也没有商家端、骑手端，所以实际上功能也是没有作用的，本来应该调整发送的处理位置，但现在暂时先不处理
                    if($from == 'admin'){
                    ...
                    if(!K::M('order/log')->create(array('status'=>-1, 'from'=>$from, 'log'=>$log, 'order_id'=>$order['order_id'])))
                        throw new \Exception("Error", 1);
                    if($refund_amount > 0){
                        if(!K::M('order/log')->create(array('status'=>-1, 'from'=>$from, 'log'=>$refund_log, 'order_id'=>$order['order_id'])))
                            throw new \Exception("Error", 1);
                    }

                    if($from=='system'||$from=='shop'){
                        if(!K::M('waimai/waimai')->update_count($order['shop_id'],'refund_order',1))
                            throw new \Exception("Error", 1);
                    }
                    ...
                    if($order['order_from'] != 'wxapp' || $from != 'member'){
                        K::M('order/order')->send_member('订单已取消', sprintf("您在[%s]下的订单(%s)，{$log}",$waimai?$waimai['title']:"-", $order_id), $order);
                    }
            ...
            try
            {
                if ($from == 'shop') {
                    if (K::M('order/order')->update($order['order_id'], array('refund_status' => 2, 'order_status' => -2, 'lasttime'=>__TIME))) {// 商家处理退款，更新退款状态已完成、订单状态 -2
                        ...
                        if ($money = K::M('order/order')->get_real_refund_amount($order['order_id'])) {//2019-03-12 修改 使用新的接口获取可退金额
                            $refund_money = $refund_amount = 0;
                            $refund_money = $order['money'];
                            //退回已付款金额到余额,这时数据库的order_status已经为-1
                            if($order['pay_status'] && ($order['amount'] > 0)){
                                ...
                                if(in_array($order['pay_code'], array('wxpay', 'alipay'))){
                                    ...
                                    $order['refund_amount'] -= K::M('order/order')->get_part_refunded_amount($order['order_id']);//2019-03-14 添加 减去已退部分
                            ...
                            if(!K::M('waimai/order/refund')->update($order['order_id'], array('reply' => $reply, 'reply_time' => time(), 'refund_price'=>$money, 'from'=>$from, 'status'=>1)))// schemas 验证了 reply_time 字段，这里不用__TIME
                            throw new \Exception("Error", 1);
                        }
                        ...
                        if(in_array($order['from'], array('waimai'))){
                            if(!K::M("{$order['from']}/order")->return_sku($order['order_id'], $order))
                                throw new \Exception("Error", 1);
                        }
                        ...
                        if($order['shop_id']){
                            if($order['from'] == 'waimai'){
                                if(!K::M('waimai/waimai')->update_count($order['shop_id'], 'orders', -1))
                                    throw new \Exception("Error", 1);
                                if($order['coupon_id']){
                                    K::M('waimai/coupon')->refund_coupon($order['coupon_id']);  //2019-03-16 注 此函数没有返回，所以不用检测是否成功
                                }
                            }else{
                                if(!K::M('shop/shop')->update_count($order['shop_id'], 'orders', -1))
                                    throw new \Exception("Error", 1);
                        ...
                        //4.0删除会员卡记录
                        if($order['peicard_id']){
                            K::M('peicard/log')->delete_by_orderId($order['order_id']); //2019-03-16 注 此函数只是执行删除的SQL，可能并不存在卡，所以不用检测返回是否成功，以免影响稳定
                        }
                        ...
                        if(!K::M('waimai/log')->create(array('from'=>$from, 'log'=>$log, 'order_id'=>$order['order_id'], 'type'=>8)))
                            throw new \Exception("Error", 1);
                        $waimai = K::M('waimai/waimai')->detail($order['shop_id']);
                        K::M('order/order')->send_member('同意退款', sprintf("您在[%s]下的订单(%s)，商家同意退款", $waimai?$waimai['title']:"-", $order['order_id']), $order);

                        if($order['first_order']){ //邀请好友
                            K::M('member/invite')->update($order['uid'], array('status'=>0)); //2019-03-16 注 此处只是修改邀请好友的状态，有可能在执行这个操作之前已经完成，或者可能会有并不存在记录等情况（因为原系统代码没有用事务等手段保证记录一定会被创建），所以这里不检测更新的状态，而这个操作的结果对整个退款结果并不是太大的影响，最多只是用户不能再享受邀请的奖励而已。
                        }
                    ...
                    if (K::M('order/order')->update($order['order_id'], array('refund_status' => 3, 'order_status' => -2, 'lasttime'=>__TIME))) {// 平台处理退款（最终）、订单状态 -2
                        ...
                        if ($money = K::M('order/order')->get_real_refund_amount($order['order_id'])) {//2019-03-12 修改 使用新的接口获取可退金额
                            ...
                            if(!K::M('waimai/order/refund')->update($order['order_id'], array('reply' => $reply, 'reply_time' => time(), 'refund_price'=>$money, 'from'=>$from, 'status'=>1)))
                                throw new \Exception("Error", 1);
                        }
                        if($order['hongbao_id']){ //退还红包
                            if(!K::M('hongbao/hongbao')->update($order['hongbao_id'], array('order_id'=>0, 'used_time'=>0, 'used_ip'=>'')))
                        ...
                        if(in_array($order['from'], array('waimai'))){
                            if(!K::M("{$order['from']}/order")->return_sku($order['order_id'], $order))
                                throw new \Exception("Error", 1);
                            if($order['coupon_id']){
                                K::M('waimai/coupon')->refund_coupon($order['coupon_id']);//2019-03-16 注 此函数无返回
                            }
                        }
                        //更新商户订单数量
                        if($order['shop_id']){
                        ...
                        //4.0删除会员卡记录
                        if($order['peicard_id']){
                            K::M('peicard/log')->delete_by_orderId($order['order_id']); //2019-03-16 注 同上一个条件中的对应语句，不作返回检测，以免影响稳定
                        }
                        ...
                        if(!K::M('waimai/log')->create(array('from'=>$from, 'log'=>$log, 'order_id'=>$order['order_id'], 'type'=>8)))
                            throw new \Exception("Error", 1);
                        $waimai = K::M('waimai/waimai')->detail($order['shop_id']);
                        K::M('order/order')->send_member('同意退款', sprintf("您在[%s]下的订单(%s)，平台同意退款", $waimai?$waimai['title']:"-", $order['order_id']), $order);
