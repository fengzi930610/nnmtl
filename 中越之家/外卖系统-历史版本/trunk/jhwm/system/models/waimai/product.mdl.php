...
class Mdl_Waimai_Product extends Mdl_Table
{
    ...
    public function items_inc_addone($filter = array(), $orderby = NULL, $p = 1, $l = 50, &$count = 0)
    {
        ...
        if(is_array($res))
        {
            ...
            {
                if(isset($item['price']) && isset($item['shop_id']) && K::M('waimai/waimai')->is_custom_mgr_shop($item['shop_id']))
    ...
    public function detail($pk, $closed = false,$useAddone=false)
    {
        ...
        if(is_array($res) && $useAddone && isset($res['price']) && isset($res['shop_id']) && K::M('waimai/waimai')->is_custom_mgr_shop($res['shop_id']))
