   ...
   // protected  $url = 'http://api.map.baidu.com/routematrix/v2/riding?output=json&ak=xi4AzWcvZ2FczMZlk4bvo0atmoqNX12h';
    ...
    public function juliinfo($lng,$lat,$lng_1,$lat_1){
        if(!$lng||!$lat||!$lng_1||!$lat_1){
            ...
            $url = "https://gg.nnmtl.com/maps/api/directions/json?key={$server_key}&origin={$lat},{$lng}&destination={$lat_1},{$lng_1}&language=zh-CN&region=VN&units=metric";
            
            //2019-02-20 添加 使用步行模式
            $url .= "&mode=walking";
    ...
    public function decode_by_location($lng,$lat){
        if(!$lng||!$lat){
            ...
            $url = "https://gg.nnmtl.com/maps/api/geocode/json?key={$server_key}&latlng={$lat},{$lng}&language=zh-CN&region=VN&result_type=point_of_interest|premise|street_address";
    ...
    public function geocode_by_addr($addr=null, $city=null)
    {
        if(!$addr || !is_string($addr) || trim($addr)===""){
            ...
            if($city)
            {
                ...
                $url = "https://gg.nnmtl.com/maps/api/place/autocomplete/json?key={$server_key}&input={$city}&language=zh-CN&region=VN&types=(cities)";
                ...
                $url = "https://gg.nnmtl.com/maps/api/geocode/json?key={$server_key}&place_id={$placeId}&language=zh-CN&region=VN";
            ...
            $url = "https://gg.nnmtl.com/maps/api/place/findplacefromtext/json?key={$server_key}&input={$addr}&inputtype=textquery&language=zh-CN&region=VN&fields=geometry,name";
