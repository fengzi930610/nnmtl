...
class Mdl_Trade_Payment extends Model
{
    ...
    //2019-03-11 添加 根据refund修改，可以指定退款金额值进行部分退款，其他相同
    //参数：
    //  $order  订单数据数组（传整个数据库记录）
    //  $amount     退款金额，小数，单位 元
    //  $reason     退款事由
    //  $msg    通常不用
    public function refund_part($order,$amount,$reason,&$msg="SUCCESS")
    {
        ...
        if($reason === "")
            return false;
        $oPayApi = $this->loadPayment($order['pay_code']);
        if(!$oPayApi)
            return false;
        $pay_log = K::M('payment/log')->pay_log_by_no($order['trade_no']);
        if(!$pay_log || $pay_log['payment']!==$order['pay_code'] || (int)$pay_log['payed']===0 || (float)$pay_log['amount']<$amount)
            return false;
        $log_data = array(
            'uid'           => $order['uid'], 
            'order_id'      => $order['order_id'], 
            'payment'       => $order['pay_code'], 
            'amount'        => $amount,
            'from'          => 'order',
            'type'          => 'refund',
            'trade_type'    => $pay_log['trade_type']
        ...
        $refund_log = K::M('payment/log')->detail($log_id);
        $params = array('refund_amount'=>$amount, 'refund_reason'=>$reason, 'order_id'=>$order['order_id']);
        $params['trade_no'] = $order['trade_no'];
        $params['total_amount'] = $order['amount'];
        $params['refund_no'] = $refund_log['trade_no'];
        $params['trade_type'] = $pay_log['trade_type'];
        $params['pay_trade_no'] = $pay_log['pay_trade_no'];
        if($trade = $oPayApi->refund($params, $msg)){
        // if($trade = ['pay_trade_no' => "debug".date("YmdHis").rand(111,999)]){//假设调用接口成功
            if(!K::M('payment/log')->set_payed($refund_log['trade_no'], $trade['pay_trade_no'])){
                return false;
            }
        }
