...
class Ctl_Order extends Ctl
{
    ...
    public function order($shop_id)
    {
        ...
        if(!$shop_id = (int)$shop_id){
            ...
            if($detail['pei_type']==0){
                ...
                //2019-01-22 新增 如果运费计算方式不为0，且有收货地址，则使用相应的配送参数重新计算运费
                 $calcType = (int)$detail['freight_calc_type'];
                if($calcType !== 0 && $m_addr)
                {
                    $useShopId = (int)$detail['shop_id'];
                    if($calcType < 0)
                        $useShopId = 0;
                    $distVal = K::M('waimai/addrdistance')->get_distance($detail['shop_id'],$m_addr['addr_id']);
                    if($distVal === false)
                        $distVal = $m_addr['juli'];
                    $distFee = K::M('waimai/freightcalcconfig')->get_fee($useShopId,$calcType,$distVal);
                    $excRates = K::M("system/config")->get("exchange_rate");
                    if($distFee !== false && $distFee>=0)
                    {
                        //使用汇率转换
                        if($distFee > 0)
                        {
                            if($excRates && isset($excRates['VND']))
                                $freight_stage = round($distFee/(float)$excRates['VND'],2);
                        }
                        else
                            $freight_stage = 0;
                    }
                    //2019-01-24 新增 加上附加费
                    if($excRates)
                        $freight_stage += round(K::M('waimai/freightcalcconfig')->get_addone(),2);
                        // $freight_stage += round(K::M('waimai/freightcalcconfig')->get_addone()/(float)$excRates['VND'],2);
                    
                    //2019-01-29 添加 将距离数据放置到页面上
                    $this->pagedata['freight_distance'] = round($distVal/1000,3);
                }
                //===================================================================================
            }else{
            ...
            $must_count = K::M('waimai/product')->items_inc_addone(array('shop_id'=>$shop_id,'closed'=>0,'is_onsale'=>1,'is_must'=>1),array('product_id'=>"DESC"),1,100,$count_count);//2019-01-26 修改 items接口改为items_inc_addone
            ...
            $product_list = K::M('waimai/product')->items_by_ids($product_ids,NULL,true);//2019-01-26 修改 使用附加费
            ...
            $spec_lists = K::M('waimai/productspec')->items_by_ids($spec_ids,NULL,true);//2019-01-26 修改 使用附加费
            ...
            foreach($cart_product_list as $pk=>$v){
                if(!$p = $product_list[$v['product_id']]){
                    ...
                    if(!$v['spec_id'] && $sp){
                        ...
                        $product_lists[$pk]['photo'] = $p['photo'];//2019-01-29 添加 将商品图片也加上
        ...
        if($huangou = K::M('waimai/huodonghuangou')->get_huangou($shop_id, ($product_price+$package_price))){
            ...
            if($p_items = K::M('waimai/product')->items_by_ids($pids,NULL,true)){//2019-01-26 修改 使用附加费
    ...
    public function preinfo()
    {
        ...
        if(!$shop_id = (int)$this->GP('shop_id')){
            ...
            $freight_stage = 0;
            $freight_distance = 0;  //2019-01-29 添加 配送距离值（公里）
            $freight_time = 0;  //2019-02-22 添加 配送时间（秒）
            ...
            foreach($cart_product_list as $pk=>$v){
                if(!$p = $product_list[$v['product_id']]){
                    ...
                    if(!$v['spec_id'] && $sp){
                        ...
                        $product_lists[$pk]['price'] = $sp['price'];
                        $product_lists[$pk]['unit'] = $p['unit'];
                        $product_lists[$pk]['photo'] = $p['photo']; //2019-01-29 添加 将图片添加到返回数据中
                        $product_price += $sp['price']  * $product_numbers[$pk];       //商品总价
                        $package_price += $sp['package_price'] * $product_numbers[$pk]; //总打包费
                        $products .= $sp['product_id'].":".$product_numbers[$pk];
                        $products .= ":".$sp['spec_id'].'&'.$v['shuxin'].',';
            ...
            if ($is_ziti == 0) {
                ...
                if($shop['pei_type']==0){
                    ...
                    //2019-01-22 新增 如果运费计算方式不为0，且有收货地址，则使用相应的配送参数重新计算运费
                    $calcType = (int)$shop['freight_calc_type'];
                    if($calcType !== 0)
                    {
                        $useShopId = (int)$shop['shop_id'];
                        if($calcType < 0)
                            $useShopId = 0;
                        $distVal = K::M('waimai/addrdistance')->get_distance($shop['shop_id'],$addr['addr_id']);//$this->msgbox->add($useShopId." ".$distVal,999)->response();
                        if($distVal === false)
                            $this->msgbox->add("无法获得距离信息",999)->response();
                        $distFee = K::M('waimai/freightcalcconfig')->get_fee($useShopId,$calcType,$distVal);
                        if($distFee === false || $distFee<0)
                            $this->msgbox->add("超出配送距离",1000)->response();
                        $excRates = K::M("system/config")->get("exchange_rate");
                        if(!$excRates || !isset($excRates['VND']) || (float)$excRates['VND']<=0)
                            $this->msgbox->add("系统未设置汇率信息",1001)->response();
                        $freight_stage = round($distFee/(float)$excRates['VND'],2);
                        //2019-01-24 新增 加上运费附加费
                        $freight_stage += round(K::M('waimai/freightcalcconfig')->get_addone(),2);
                        // $freight_stage += round(K::M('waimai/freightcalcconfig')->get_addone()/(float)$excRates['VND'],2);

                        //2019-01-29 添加 赋值配送距离
                        $freight_distance = round($distVal/1000,3);

                        //2019-02-22 添加 赋值配送时间
                        $freight_time = K::M('waimai/freightcalcconfig')->get_freight_time($distVal);
                    }
                    //===================================================================================
            ...
            if($huangou = K::M('waimai/huodonghuangou')->get_huangou($shop_id, ($product_price + $package_price))){
                $pids = array_keys($huangou['products']);
                if($hgpro_items = K::M('waimai/product')->items_by_ids($pids,NULL,true)){//2019-01-26 修改 使用附加费
            ...
            $data = array(
                ...
                'freight_distance' => $freight_distance,//2019-01-29 添加 将配送距离也返回
                'freight_time' => $freight_time,////2019-02-22 添加 将配送时间也返回
                'youhui'=>$youhui,
            ...
            if($params = $this->checksubmit('params')){
                ...
                if(!$shop_id = (int) $params['shop_id']){
                    ...
                    if($shop_detail['yy_status'] == 0 || $shop_detail['yysj_status'] == 0){
                        ...
                        $product_list = K::M('waimai/product')->items_by_ids($product_ids,NULL,true);//2019-01-26 修改 使用附加费
                        $must_count = K::M('waimai/product')->items_inc_addone(array('shop_id'=>$shop_id,'closed'=>0,'is_onsale'=>1,'is_must'=>1),array('product_id'=>"DESC"),1,100,$count_count);//2019-01-26 修改 items修改为items_inc_addone
                        ...
                        //2019-01-24 新增 标识是否是按距离计算运费模式
                        $bIsCalcFreight = false;
                        $freightAddoneTrans = 0.00;
                        ///============================
                        //2019-02-22 新增 记录预计配送时间
                        $freightTime = 0;
                        //===========================
                        ...
                        if($params['pei_type'] == 3){
                            ...
                            else if((int)$shop_detail['pei_type']===0 && (int)$shop_detail['freight_calc_type']!==0)
                            {
                                //2019-01-22 新增 如果运费计算方式不为0，重新计算运费
                                $calcType = (int)$shop_detail['freight_calc_type'];
                                $useShopId = (int)$shop_detail['shop_id'];
                                if($calcType < 0)
                                    $useShopId = 0;
                                $distVal = K::M('waimai/addrdistance')->get_distance($shop_detail['shop_id'],$addr_detail['addr_id']);
                                if($distVal === false)
                                    $this->msgbox->add("无法获得距离信息",1)->response();
                                $distFee = K::M('waimai/freightcalcconfig')->get_fee($useShopId,$calcType,$distVal);
                                if($distFee === false || $distFee<0)
                                    $this->msgbox->add("超出配送距离",1)->response();
                                $excRates = K::M("system/config")->get("exchange_rate");
                                if(!$excRates || !isset($excRates['VND']) || (float)$excRates['VND']<=0)
                                    $this->msgbox->add("系统未设置汇率信息",1)->response();
                                $freight = round($distFee/(float)$excRates['VND'],2);
                                $data_order['is_calc_freight_mod'] = 1;
                                //2019-01-24 新增 运费附加费要记录到waimai_order中
                                $freightAddoneTrans = round(K::M('waimai/freightcalcconfig')->get_addone(),2);
                                // $freightAddoneTrans = round(K::M('waimai/freightcalcconfig')->get_addone()/(float)$excRates['VND'],2);
                                $freight += $freightAddoneTrans;
                                $bIsCalcFreight = true;

                                //2019-02-22 新增 记录配送距离
                                $freightTime = K::M('waimai/freightcalcconfig')->get_freight_time($distVal);
                                //===================================================================================
                            }
                            ...
                            //2019-03-04 添加 将收货地址的图片也复制到订单中
                            $data_order['addr_photo'] = $addr_detail['photo'];
                        }
                        ...
                        if($hg_cart = $this->get_hg_cart($shop_id)){
                            if(!$huangou = K::M('waimai/huodonghuangou')->get_huangou($shop_id, ($product_price+$package_price))){
                                ...
                                $hgpro_items = K::M('waimai/product')->items_by_ids($pids,NULL,true);//2019-01-26 修改 使用附加费
                                foreach ($hg_pros as $k => $v) {
                                    if(!$hp = $hgpro_items[$v['product_id']]){
                                        $this->msgbox->add('商品不存在或已删除！',305)->response();
                                    }else if($hp['shop_id'] != $shop_id){
                        ...
                        //2019-02-22 新增 如果是计算运费模式，且设置了配送时间，则做配送时间最小值判断
                        if($freightTime > 0 && (int)$pei_time > 0 && $pei_time<time()+$freightTime)
                            $this->msgbox->add("请选择".(date("H:i",time()+$freightTime))."后的配送时间",405)->response();
                        //==========================================================
                        ...
                        if($order_id = K::M('order/order')->create($data_order)) {
                            ...
                            //2019-01-24 新增 如果是计算运费模式，则加上附加费的值
                            if($bIsCalcFreight)
                                $data_waimai['freight_addone'] = $freightAddoneTrans;
                            else
                                $data_waimai['freight_addone'] = 0;
                            //=================================================

                            //2019-02-22 新增 如果是计算运费模式，且没有设置配送时间，则将配送预计时间缓存起来，订单未完成时要显示这个时间值
                            if($freightTime > 0 && (int)$pei_time === 0)
                                K::M('order/order')->cacheFreightTime($order_id,$freightTime);
                            //==========================================================
                            ...
                            //==========2019-01-18 添加订单是否需要中文到货通知服务=========
                            if(isset($params['use_arrival_srv']) && (int)$params['use_arrival_srv'] === 1)
                            {
                                //创建中文到货通知数据
                                $srvData = [
                                ...
                                K::M('order/srv/arrival')->create($srvData);
                            }
                            //===========================================================
