...
class Ctl_Product extends Ctl
{
    ...
    { 
        ...
        if(!$items = K::M('waimai/product')->items_inc_addone($filter,$orderby,$page, $limit, $count)){//2019-01-26 修改 items接口改为items_inc_addone
            $items = array();
        }
        /*$shop_ids = array();
    ...
    { //商家
        ...
        if(K::M('waimai/config')->get_hot_show()){
            foreach($items as $k=>$v){
                $items[$k]['products'] = K::M('waimai/product')->items_inc_addone(array('shop_id'=>$v['shop_id'],'is_hot'=>1,'closed'=>0,'is_onsale'=>1),null,1,4);//2019-01-26 修改 items接口改为items_inc_addone
            }
        }
    ...
    public function detail($product_id)
    {
        ...
        if(!$product_id = (int)$product_id){
        ...
        }else if(!$detail=K::M('waimai/product')->detail($product_id,false,true)){//2019-01-26 修改，使用附加费
            ...
            if($spec_list=K::M('waimai/productspec')->items_inc_addone(array('product_id'=>$product_id))){//2019-01-26 修改 items改为items_inc_addone
            ...
            $this->pagedata['items2'] = K::M('waimai/product')->items_inc_addone($filter,array('price'=>'asc'));//2019-01-26 修改 items接口改为items_inc_addone
            ...
            $must_count = K::M('waimai/product')->items_inc_addone(array('shop_id'=>$detail['shop_id'],'closed'=>0,'is_onsale'=>1,'is_must'=>1),array('product_id'=>"DESC"),1,100,$count);//2019-01-26 修改 items接口改为items_inc_addone
            $must_ids = array();
