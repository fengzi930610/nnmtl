...
class Ctl_Shop extends Ctl
{
    ...
    public function detail($shop_id = null)
    {
        ...
        $filter = array('shop_id'=>$shop_id,'closed'=>0,'is_onsale'=>1,'is_spec'=>0,'product_id'=>'NOTIN:'.implode(',',$disc_pids));
        $this->pagedata['items2'] = $items2 = K::M('waimai/product')->items_inc_addone($filter,array('price'=>'asc'),1,10);//2019-01-26修改，把items接口改为items_inc_addone
        $detail['avg_time'] = $detail['pei_time'];
        ...
        $must_count = K::M('waimai/product')->items_inc_addone(array('shop_id'=>$shop_id,'closed'=>0,'is_onsale'=>1,'is_must'=>1),array('product_id'=>"DESC"),1,100,$count);//2019-01-26修改，把items接口改为items_inc_addone
        $must_ids = array();
        foreach($must_count as $k=>$v){
            $must_ids[] = $v['product_id'];
        }
        ...
        if($tj_products = K::M('waimai/product')->items_inc_addone($tj_filter, array('product_id'=>'desc'), 1, 50, $tj_count)){//2019-01-26修改，把items接口改为items_inc_addone
        ...
        //2019-01-29 添加 如果配送费是按距离计算模式，则获取对应的配送费配置参数
        $fcCfg = [];
        if((int)$detail['freight_calc_type'] !== 0)
        {
            $fcCfg = K::M('waimai/freightcalcconfig')->get_data((int)$detail['freight_calc_type']>0?$detail['shop_id']:0,(int)$detail['freight_calc_type']);
            $excRate = K::M('system/config')->get("exchange_rate");
            if(is_array($excRate) && isset($excRate['VND']))
            {
                $fcCfg['start_fee'] = round((float)$fcCfg['start_fee']/(float)$excRate['VND'],2);
                $fcCfg['fee_base'] = round((float)$fcCfg['fee_base']/(float)$excRate['VND'],2);
            }
            $fcCfg['start_distance'] = (int)$fcCfg['start_distance']/1000;
            $fcCfg['distance_base'] = (int)$fcCfg['distance_base']/1000;
            //2019-01-30 添加 将附加费也显示上去
            $fcCfg['start_fee'] += K::M('waimai/freightcalcconfig')->get_addone();
        }
        $this->pagedata['fc_cfg'] = $fcCfg;
        //===============================================================
    ...
    public function loadgoods($page=1)
    {
        ...
        //2019-01-26 修改 将调用items()改为调用items_inc_addone()
        if(!$items = K::M('waimai/product')->items_inc_addone($filter,$orderby,$page, $limit, $count)){
            $items = array();
        }
    ...
    public function loaditems($page=1)
    {
        ...
        $html = $this->output(true);
        $this->msgbox->set_data('html', mb_convert_encoding($html,'UTF-8','UTF-8'));
        $this->msgbox->json();
    }
    ...
    public function get_spec()
    {
        if(!$porduct_id = (int)$this->GP('product_id')){
            ...
            if($spec_list = K::M('waimai/productspec')->items_inc_addone(array('product_id'=>$porduct_id))){//2019-01-26 修改 items改为items_inc_addone
    ...
    public function cart()
    {
        ...
        //print_r($items);die;
        $products = K::M('waimai/product')->items_by_ids($product_ids,NULL,true);//2019-01-26修改，使用附加费
        $specs = K::M('waimai/productspec')->items_by_ids($spec_ids,NULL,true);//2019-01-26 修改 使用附加费
        $nums = array();
        foreach($items as $k=>$v){
            foreach($v as $k1=>$v1){
    ...
    public function search($shop_id = null)
    {
        if(!$shop_id = (int)$shop_id){
           ...
           /* $area_price = K::M('waimai/waimai')->get_shipping_fee($detail['area_polygon'], $lat, $lng);
            ...
            $this->pagedata['items2'] = K::M('waimai/product')->items_inc_addone($filter,array('price'=>'asc'),1,10);//2019-01-26修改，把items接口改为items_inc_addone
