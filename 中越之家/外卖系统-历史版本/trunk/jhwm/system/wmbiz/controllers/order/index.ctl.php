...
class Ctl_Order_Index extends Ctl
{
    ...
    public function receive($page=1)
    {
        ...
        if ($keyword = (int)$this->GP('keyword')) {// 头部搜索
            ...
            //2019-03-02 添加 订单号搜索处理
            $keyword = trim($this->GP('keyword'));
            if(preg_match('/^\d{15,}$/', $keyword))
                $filter[':OR']['order_id'] = substr($keyword, 14);
            //==========================
        }
    ...
    public function delivery_confirm($order_id){
        if(!$order_id){
            ...
            if(K::M('order/order')->update($order_id,array('order_status'=>4,'shop_confirm_time'=>time()))){    //2019-03-06 修改 把时间写入订单中
    ...
    public function refund_part($id)
    {
        ...
        try
        {
            ...
            if(!$order || (int)$order['shop_id']!==(int)$this->shop_id)
            ...
            if(!K::M('order/order')->refund_ex($id,$amount,$remark,"shop"))
