    ...
    public function edit_save()
    {
        if ($_POST) {
            ...
            if ($this->waimai_shop['pei_type'] != 0) {// 0:自己送，1:跑腿送,  2:代购(仅仅外卖), 3:用户自提单
                ...
                //20190113 保存半径设置值
                if(is_array($data['area_price']))
                {
                    foreach($data['area_price'] as $key => &$vvv)
                    {
                        if(isset($data['area_price'][$key]['set_radius']))
                        {
                            $data['area_price'][$key]['set_radius'] = (float)$data['area_price'][$key]['set_radius'];
                            if($data['area_price'][$key]['set_radius'] < 0)
                                $data['area_price'][$key]['set_radius'] = 0;
                        }
                        else
                            $data['area_price'][$key]['set_radius'] = 0;
                        unset($vvv);
