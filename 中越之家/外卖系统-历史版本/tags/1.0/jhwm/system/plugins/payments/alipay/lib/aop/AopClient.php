...
class AopClient {
	...
	protected function sign($data, $signType = "RSA2") {
		//$this->rsaPrivateKeyFilePath = __CORE_DIR.'rsa_private_key.20151212.pem';
		if($this->checkEmpty($this->rsaPrivateKeyFilePath)){
	        ...
	        //     $key_len = strlen($priKey);
	        //     $pem_key = "-----BEGIN RSA PRIVATE KEY-----\n";
	        //     for($i=0; $i<$key_len; $i=$i+64){
	        //         $pem_key .= substr($priKey, $i, 64)."\n";
	        //     }
		...
		if ("RSA2" == $signType) {
			...
			if(version_compare(PHP_VERSION, '5.4.8', '<')){
				openssl_sign($data, $sign, $res, sha256WithRSAEncryption);
			}else{
				openssl_sign($data, $sign, $res, OPENSSL_ALGO_SHA256);
			}
		} else {
	...
	protected function logCommunicationError($apiName, $requestUrl, $errorCode, $responseTxt) {
		...
		$key = 'payment-alipay.aop.'.date('Ymd');
        K::M('system/logs')->log($key, $logData);
	}
...
//        print_r($totalParams);
	...
	function verify($data, $sign, $rsaPublicKeyFilePath, $signType = 'RSA2') {
		...
		if ("RSA2" == $signType) {
			...
			if(version_compare(PHP_VERSION, '5.4.8', '<')){
				$result = (bool)openssl_verify($data, base64_decode($sign), $res, sha256WithRSAEncryption);
			}else{
				$result = (bool)openssl_verify($data, base64_decode($sign), $res, OPENSSL_ALGO_SHA256);
			}			
		} else {
			...
			if (!$this->checkEmpty($responseSubCode) || ($this->checkEmpty($responseSubCode) && !$this->checkEmpty($signData->sign))) {
				...
				if (!$checkResult) {
					...
					if (strpos($signData->signSourceData, "\\/") > 0) {
						...
						throw new Exception("check sign Fail2! [sign=" . $signData->sign . ", signSourceData=" . $signData->signSourceData . "]");
					}
