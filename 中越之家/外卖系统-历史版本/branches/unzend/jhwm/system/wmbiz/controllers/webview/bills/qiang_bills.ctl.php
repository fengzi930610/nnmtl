...
class Ctl_Webview_Bills_Qiang_Bills extends Ctl {
      ...
      //  0:待审核 1:已通过 2:以拒绝 4：已完成
        ...
        if(defined("HAVE_QIANG")){
            ...
            $count = array(
                ...
                'day'=>date('Y-m-d'),
                'yes_bl'=>($qiang_count_order/($qiang_count_order+$qiang_count_order_yichang))*100,
                'no_bl'=>($qiang_count_order_yichang/($qiang_count_order+$qiang_count_order_yichang))*100,
            );
        ...
        $this->tmpl = 'webview/bills/qiang_bills/index.html';
    }


    //历史对账
    public function history(){
        $this->tmpl = 'webview/bills/qiang_bills/history.html';
    }
    ...
    public function loadbills($page=1){
        ...
        $this->tmpl = 'webview/bills/qiang_bills/loadbills.html';
    ...
    public function detail($bills_id){
        if($day = $this->GP('day')){
            ...
            if(!$detail){
                ...
                $this->msgbox->set_data('forward',$this->mklink('webview/bills/qiang_bills/history',array(),array(),'wmbiz'));
            }
            ...
            $this->tmpl = 'webview/bills/qiang_bills/detail.html';
        }else{
            if(!$bills_id){
    ...
    public function loadorder($page=1){
        ...
        $this->msgbox->set_data('loadst', $loadst);
        $this->pagedata['items'] = $items;
        $this->tmpl = 'webview/bills/qiang_bills/loadorder.html';
        $html = $this->output(true);
        $this->msgbox->set_data('html',$html);
        $this->msgbox->json();
    }
    ...
    public function orderdetail($order_id){
        if(!$order_id){
            ...
            $this->tmpl = 'webview/bills/qiang_bills/orderdetail.html';
        }
    }
