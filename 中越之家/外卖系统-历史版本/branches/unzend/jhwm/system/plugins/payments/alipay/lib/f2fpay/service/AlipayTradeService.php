...
function json_encode_ex($value,$ext = '')
{
...
class AlipayTradeService {
    ...
    function wapPay($builder,$return_url,$notify_url) {
        ...
        // 首先调用支付api
        $response = $this->wap_aopclientRequestExecute ($request,true);
    ...
    function wap_aopclientRequestExecute($request,$ispage=false) {
	...
	function writeLog($text) {
		K::M('system/logs')->log('alipay.aop.'.date('Ymd'), $text);
	}
	...
	function create_erweima($content, $size = '200', $lev = 'L', $margin= '0') {
		...
		$url = 'http://api.wxzhu.com/tools/qrcode?data=xxx&size='.$size;
		$image = '<img src="'.$url.'"  widht="'.$size.'" height="'.$size.'" />';
		return $image;
	}
