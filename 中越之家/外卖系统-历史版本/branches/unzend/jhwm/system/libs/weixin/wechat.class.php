...
{   
    ...
    public function __construct($crypt=null)
    {
        $this->Crypt = $crypt;
        ...
        if($this->Crypt){
            ...
            $this->_nonce = $nonce;
            $errCode = $this->Crypt->decryptMsg($msg_sign, $timeStamp, $nonce, $content, $xmldata);
            if($errCode){
                return false;
            }
    ...
    private function _replyData($msg, $msgType)
    {
        ...
        $xmldata = $xml->asXML();
        // 记录日志
        K::M('weixin/log')->log($this->data['ToUserName'], $xmldata, '_replyData');
        if($this->Crypt){
            $content = $xmldata;
            $errCode = $this->Crypt->encryptMsg($xmldata, __TIME, $this->_nonce, $content);
            if($errCode){
                return false;
            }
            $xmldata = $content;
...
class WeChatClient {
    ...
    public function getUserInfoById($uid, $lang = '') {
        ...
        //$url = "https://api.weixin.qq.com/sns/userinfo?access_token=$access_token&openid=$uid&lang=$lang";
        $res = json_decode(self::get($url), true);
    ...
    */
    public function sendTempMsg($openid, $template_id, $url, $params, $miniprogram)
    {   
        $access_token = $this->getAccessToken();        
    ...
    public function getTempId($TMID)
    {
        $access_token = $this->getAccessToken();
        $api_url = self::$_URL_API_ROOT."/cgi-bin/template/api_set_industry?access_token=$access_token";
        if(!$res = self::post($api_url, '{"industry_id1":"1","industry_id2":"4"}')){
            return false;
        }
        $api_url = self::$_URL_API_ROOT."/cgi-bin/template/api_add_template?access_token=$access_token";
        if($res = self::post($api_url, '{"template_id_short":"'.$TMID.'"}')){
            if($result = json_decode($res, true)){
                if($result['errcode'] == 0 && $result['template_id']){
