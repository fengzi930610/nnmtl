    ...
    if(checkIsWeixin()){ //微信获位置坐标
                        ...
                        geocoder(UxLocation.lng, UxLocation.lat, function(ret){
                            if(ret.infocode ==10000){
                                if(ret.regeocode.formatted_address!=undefined){
                                    UxLocation['addr'] = ret.regeocode.formatted_address;
                                    UxLocation["error"] = 0;
                                    setUxLocation(UxLocation);
                                    callback(UxLocation);
                                    return true;
                    ...
                    if(e.error==0){
                      var locations = e.data.location.split(',');
                        ...
                        geocoder(UxLocation.lng, UxLocation.lat, function(ret){
                         ...
                         callback({"error":-1, "message":data.message});
                         return false;
                    }
                },'json');
            });      //返回定位出错信息
        ...
        geolocation.getCurrentPosition(function(r){
            if(this.getStatus() == BMAP_STATUS_SUCCESS){
                UxLocation["lng"] = r.point.lng.toFixed(6);
                UxLocation["lat"] = r.point.lat.toFixed(6);
                setUxLocation(UxLocation);
                geocoder(UxLocation.lng, UxLocation.lat, function(ret){
                    if(ret.infocode ==10000){
                        if(ret.regeocode.formatted_address!=undefined){
                ...
                callback({"error":-1, "message":this.getStatus()});
            }        
